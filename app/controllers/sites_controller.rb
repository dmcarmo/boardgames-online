class SitesController < ApplicationController
  ...
  def update_games
    ...
    results.keys.each do |item|
      ...
      unless @site.games.include?(game)
        @site.games << game
        game_site = @site.game_sites.where(game_id: game.id).first
        game_site.game_url = results[item][:game_url]
        game_site.save
      end
    end
  ...
  def boardgamearena(page, results)
    page.search("#gamelist_itemrow_inner_all .gamelist_item").each do |element|
      ...
      game_url = @site.url + element.search("@href").text
      bgg_url = ""
      bgg_id = ""
      results[name.to_sym] = { bgg_url: bgg_url, bgg_id: bgg_id, game_url: game_url }
    end
    results
  end
  ...
  def boiteajeux(page, results)
    page.search(".jeuxRegles").each do |element|
      name = element.search("text()").text.strip.split("\r").first.capitalize
      game_url = @site.url
...
# list of sites
# implemented:
#   BoardGameArena, Boiteajeux, Happy Meeple, VASSAL, Yucata, Tabletopia
# to implement:
#   Boardgamecore, BoardGamePlay, BoardGamingOnline, MaBi Web, Online Terra Mystica, Orderofthehammer, rr18xx,
#   SlothNinja, Web Diplomacy,
